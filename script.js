...
function scrollToSelected() {
  const maxScroll = (aliens.length - visibleCards) * cardWidth;
...
function nextSlide() {
  if (selectedIndex < aliens.length - 1) {
    selectedIndex++;
    updateSelectedCard();
    scrollToSelected();
  }
...
function closePopup() {
  ...
  // RESET POPUP BOX
  detailBox.style.border = "1px solid rgba(0, 255, 136, 0.4)";
  detailBox.style.boxShadow = "0px 0px 30px rgba(0, 255, 136, 0.4)";

  // RESET IMAGE
  detailImg.style.border = "3px solid #00ff88";
  detailImg.style.boxShadow = "0px 0px 20px rgba(0, 255, 136, 0.6)";
}
...
document.addEventListener("keydown", (event) => {
  ...
  // PRESS ENTER TO OPEN SELECTED ALIEN
  if (event.key === "Enter") {
    openPopup(selectedIndex);
  }
});
